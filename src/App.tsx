...
function App() {
  ...
  const setMax = (values: TireDataForm): boolean | void =>
    checkMax(values, minTireData) && setMaxTireData(values);
  const setMin = (values: TireDataForm): boolean | void =>
    checkMax(maxTireData, values) && setMinTireData(values);
  ...
  const [tires, setTires] = useState<Tire[]>([minTireData, maxTireData]);
  ...
  useEffect(() => {
    ...
    const selectedTireData1 = selectedTireFromHash(
      params.get('selectedTire1') || ''
    );
    const selectedTireData2 = selectedTireFromHash(
      params.get('selectedTire2') || ''
    );
    ...
    selectedTireData1 &&
      selectedTireData2 &&
      setTires([selectedTireData1, selectedTireData2]);
  }, []);
  ...
  useEffect(() => {
    ...
    if (tires[0])
      params.set(
        'selectedTire1',
        `${tires[0].width}/${tires[0].aspectRatio}/${tires[0].wheelDiameter}`
      );
    if (tires[1])
      params.set(
        'selectedTire2',
        `${tires[1].width}/${tires[1].aspectRatio}/${tires[1].wheelDiameter}`
      );
    params.set(
      'tire1',
      `${minTireData.width}/${minTireData.aspectRatio}/${minTireData.wheelDiameter}/${minTireData.heightLimit}`
    );
    params.set(
      'tire2',
      `${maxTireData.width}/${maxTireData.aspectRatio}/${maxTireData.wheelDiameter}/${maxTireData.heightLimit}`
    );
    window.location.hash = params.toString();
  }, [tires, minTireData, maxTireData]);

  const changeTires = (tires: Tire[]) => [
    ...tires.slice(1, tires.length),
    tires[0],
  ];

  return (
    <main style={{ width: '850px', margin: 'auto' }}>
      <h2 style={{ fontWeight: '500', fontSize: '30px' }}>
        Tire Size and Height Comparison Calculator
      </h2>
      <Panel
        header={
          <span>
            Compare tires by size and calculate revolutions per unit of
            distance.
          </span>
        }
      >
        ...
        >
          <Panel
            header={
              <span>
                Minimum <TireHeight values={minTireData} />
              </span>
            }
          >
            <TireForm
          ...
          <Panel
            header={
              <span>
                Maximum <TireHeight values={maxTireData} />
              </span>
            }
          >
            <TireForm
          ...
          header={
            <div style={{ display: 'flex' }}>
              ...
              <div style={{ marginLeft: 'auto' }}>
                <button onClick={() => setTires(changeTires)}>Swap</button>
              </div>
            </div>
          }
